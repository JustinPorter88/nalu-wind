...
MotionAirfoilSMDKernel::MotionAirfoilSMDKernel(const YAML::Node& node)
...
KOKKOS_FUNCTION
    mm::TransMatType
    MotionAirfoilSMDKernel::build_transformation(
...
KOKKOS_FUNCTION
mm::ThreeDVecType
MotionAirfoilSMDKernel::compute_velocity(
  const double& time,
...
double MotionAirfoilSMDKernel::get_cur_angle(const double time)
{
  double motionTime = (time < endTime_)? time : endTime_;
...
double MotionAirfoilSMDKernel::get_cur_ang_vel(const double time)
{
  double motionTime = (time < endTime_)? time : endTime_;
